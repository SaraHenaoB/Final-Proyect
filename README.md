...
**Equipo MetricEdge S.A.S** · _Predictive analytics for smart business decisions._
...
Final-Proyect/
├── data/
│   ├── raw/                                  # CSV originales + README con el link de Kaggle
│   └── processed/                            # Generado por etl.py (*_clean.csv + interacciones_clean.csv)
├── notebooks/
│   ├── 01_eda_primario.ipynb                 # Diagnóstico de calidad de datos
│   ├── 02_eda_profundo.ipynb                 # Viabilidad de negocio + feature engineering
│   ├── 03_modelo_contenido.ipynb             # TF-IDF + similitud coseno
│   ├── 04_modelo_colaborativo.ipynb          # ALS (implicit)
│   ├── 05_modelo_hibrido.ipynb               # Contenido + ALS + re-ranking (LightGBM/LogReg/RF)
│   ├── hybrid_candidates_dataset.csv         # Dataset de entrenamiento del re-ranker
│   └── hybrid_val_full_candidates.csv        # Candidatos completos de validación
├── src/
│   ├── etl.py                                # Aplica las 8 reglas de limpieza confirmadas
│   ├── content_based.py                      # Clase ContentBasedEngine
│   └── generar_candidatos_hibrido.py         # Genera los 2 CSV de candidatos del híbrido
├── models/
│   ├── content_cosine_sim.joblib
│   ├── content_tfidf_vectorizer.joblib
│   └── content_scaler.joblib
├── docs/
│   ├── 01_readme_eda_primario.md
│   ├── 02_readme_eda_profundo.md
│   ├── 03_readme_modelo_contenido.md
│   └── 05_readme_modelo_hibrido.md
├── requirements.txt
├── .gitignore
└── README.md
```
...
| Retraso logístico               | **14.9%** de las órdenes con dato de entrega llegó fuera de la fecha estimada _(se calcula sobre las órdenes que sí tienen `delivery_days`, no sobre el total — una orden `Cancelled`/`Pending` nunca tiene fecha de entrega, incluirla en el denominador subestimaría el problema)_ |
...
3. (Opcional) Revisa el diagnóstico de calidad de datos, sobre los CSV crudos:

   jupyter nbconvert --to notebook --execute --inplace notebooks/01_eda_primario.ipynb

4. Corre el ETL desde la raíz del repo (aplica las reglas confirmadas en el paso anterior,
   lee data/raw/_.csv, escribe data/processed/_\_clean.csv):

   python src/etl.py

5. (Opcional) Revisa la viabilidad de negocio, sobre los datos ya limpios:

   jupyter nbconvert --to notebook --execute --inplace notebooks/02_eda_profundo.ipynb

6. Entrena y evalúa los modelos, en este orden:

   jupyter nbconvert --to notebook --execute --inplace notebooks/03_modelo_contenido.ipynb
   jupyter nbconvert --to notebook --execute --inplace notebooks/04_modelo_colaborativo.ipynb
   jupyter nbconvert --to notebook --execute --inplace notebooks/05_modelo_hibrido.ipynb
...
Para dudas, sugerencias o reportar inconsistencias, abrir un _issue_ en el repositorio o contactar al equipo MetricEdge S.A.S.
